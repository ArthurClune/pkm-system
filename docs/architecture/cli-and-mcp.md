...
[backend.md](backend.md#http-api-reference). Failures and their fixes are
...
embedded assistant's `policy.py` splits the tools along this read/write line —
reads auto-allowed, writes confirm-gated (see
...
`pkm local check` has no MCP counterpart: it audits every `/api/local/` link in
block text against the host's filesystem (`GET /api/local/check`, see
[backend.md](backend.md#local-documents)), an operator diagnostic. Its exit
...
bare dict, so the planners and renderers downstream read typed attributes and a
field that drifts is a pyrefly error. A 2xx body that doesn't satisfy its model
raises `ResponseSchemaError`, naming the endpoint and the offending field path.
That is an `ApiError`, so the CLI still exits 1 with one line on stderr. Unknown
extra fields are ignored, so a newer server stays usable from an older CLI.

## Shared write workflows

`client/workflows.py` (Shell) holds the write workflows the CLI and MCP server
both perform: `save_blocks`, `edit_block`, `apply_batch`, `upload_and_link`, and
the `default_page_title` rule (today's daily note). The ordering invariants below
— validate before any I/O, resolve the parent before uploading, page creation
inside the same batch — live there, so an ordering fix lands once. Each shell
phrases the return values itself, the CLI by printing and the MCP tools by
returning strings.

`upload_and_link`, behind both the CLI's `pkm upload` and the MCP
`upload_asset` tool, resolves and validates the destination page and parent
...
then fails, `upload_and_link` compensates with `DELETE /api/assets/{sha256}`,
but only when `existing` was `false`: a dedup hit's sha may already be
referenced by unrelated blocks. The asset store itself is covered in
...
`UID_RE` (`contracts/ops.py`) itself still *accepts* a leading `-` or `_`, so a
block whose uid starts with one stays addressable for updates and moves. Any
tightening of `UID_RE` must apply to newly-minted uids only.
...
plus a "missing" flag; blocks are all a planner needs, and the only part of a
page payload a missing page can honestly stand in for. The shared workflows
(`save_blocks`, `apply_batch`, `upload_and_link`) prepend a `create_page` op
(`planning.create_page_ops`) to the same `OpBatch` the planned blocks ride in,
so a batch that fails validation leaves neither the page nor its blocks behind.
The lookup uses `refs.normalize_title(title)`, because a page is only ever
stored under its normalized spelling and a verbatim lookup would report a false
"missing". The ops built from that call still carry the caller's spelling, which
the server normalizes at the same `get_or_create_page` choke point onto the
identical row.
...
group is fetched, because no user-visible output in this project truncates
silently. The route sorts sources by `(updated_at DESC, title)`, so a concurrent
write can shift a source across a page boundary mid-fetch.
`_fetch_backlinks_once` detects that skew and `get_backlinks`
restarts from offset 0, bounded by `_BACKLINK_MAX_ATTEMPTS` and raising rather
than ever returning a possibly skipped or duplicated set.

## Pure planners

Three top-level Core modules sit under no shell: `planning.py` plans a write,
`batch.py` plans a multi-command batch on top of it, `render.py` renders API
payloads to terminal markdown. The CLI, the MCP server and
`client/workflows.py` all use all three.
...
Every `Planner` method takes a parent *uid*, already resolved. Turning a parent
*spec* into one stays with the caller. Aliases and in-batch uids are batch
bookkeeping, and `batch.py` is what knows them.
...
`Planner`, the fetched pages, the alias map, and the uids created so far in this
batch. A parent spec resolves by its form:
...
The memo follows the same level-and-text rule as the fetched-page match, so a
heading resolves to the same parent whether it came from the fetched page or
from earlier in the batch. An `order_idx` of 0 is what an off-page parent gets,
because no fetched page can supply its child count.
...
counting from the page's original child count.
...
The marked form is the same level-and-text rule `--parent` uses, so the two
specs cannot disagree about which `Notes` they mean. The bare form is the
lenient one, for callers that don't know how a section is marked up. Text still
matches exactly; only the heading level is lenient.

A miss raises `RenderError` listing the page's headings *with* their level
markers, so the error text tells you which spelling to ask for next. The `{1,3}`
bound on `_SECTION_MARKER` matches the app's whole heading domain
(`HEADING_COMMANDS` in `web/src/outline/slashCommands.ts` offers h1-h3 only), so
a `####` spec is read as bare text, which still finds the block by exact text
whatever its level.
...
Exclusions: `#Tag` (no space), `#### ` and deeper, and multi-line text, which
stays verbatim in one block; [docs/cli.md](../cli.md#writing) states them for
`save`, `batch` and `update` alike. The `-D`/`-T`/`mark=` task-marker paths use
`plan_mark`, not `plan_update`, and never emit `set_heading`: the text they read
back is already bare, so splitting it would demote a real heading.

The round trip is `pkm get`/`get_page`/`get_block` only. The renderers behind
`pkm todos`, `query`, `refs` and `search` (`render_groups`, `render_backlinks`,
`render_search`) print bare text — `item.text` for the grouped verbs, `snippet`
for search — because their response models (`GroupItem`, `BacklinkItem`,
