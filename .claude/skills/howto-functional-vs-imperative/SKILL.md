...
description: Use when designing new modules, refactoring files that mix business logic with I/O, or when unsure whether code belongs in Functional Core or Imperative Shell - not needed for routine edits, where the FCIS rules in CLAUDE.md suffice
...
The FCIS pattern applies to all code; the short rules in CLAUDE.md cover routine edits. Invoke this skill for **structural decisions**:

- Designing a new module or package layout
- Refactoring a file that mixes business logic with I/O
- Reviewing code for architectural decisions
- A classification is genuinely unclear
