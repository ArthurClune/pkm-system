...
updated_at: 2026-07-30T07:57:10Z
---
...
- [ ] Add a themed `:focus-visible` outline to the three ghost icon buttons
- [ ] Audit the remaining bare `<button>` classes for the same omission
- [ ] Guard it in `web/src/styles.test.ts` (text-level drift guard; note `ruleFor` needs the full comma-separated selector list as one string)
