...
## Checklist
- [ ] Normalize and byte-truncate filenames to a safe component limit at upload/import, preserving a usable extension
- [ ] Apply the same defensive truncation in the exporter (existing rows may already be unsafe)
- [ ] Handle '.'/'..' and empty-after-sanitization names explicitly
- [ ] Regression: overlong ASCII filename upload → export succeeds
- [ ] Regression: multibyte Unicode where char count is under the limit but UTF-8 byte count is over
- [ ] Regression: dot names, collisions after truncation, extension preservation
