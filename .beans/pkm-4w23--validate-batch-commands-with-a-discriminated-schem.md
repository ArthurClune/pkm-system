...
Also removed two now-redundant runtime checks that the schema subsumes: `_page()`'s "command needs a 'page' param" (page is now a required, non-empty schema field) and the outline branch's "outline needs non-empty 'items'" (now `Field(min_length=1)` on `OutlineParams.items`, plus a `model_validator` -- see review fix below -- catching the flattened-empty case `min_length` alone misses); `_nested_items` dropped its own type-checking raise since the recursive schema now guarantees every leaf is a string and every branch a list before it ever runs.

New tests (build/CLI/MCP layers): non-object batch items, non-object `params`, missing/wrong-typed fields, negative `index`, unparseable `index`, malformed nested outline items, empty `items` (top-level and flattened-empty nestings like `[[]]`), unknown param keys (typos), the offending index being reported for a multi-item batch, `validate_batch` returning parsed commands for a valid batch, `plan_batch` itself rejecting non-object items/missing fields/negative index when called directly, and end-to-end CLI/MCP tests for non-object items, missing/wrong-typed fields, negative index, unknown alias, flattened-empty outline items, and a schema failure in a later command leaving an earlier command's brand-new page uncreated (proving validation runs before any I/O).

**Review fix round 1:** `OutlineParams.items: Field(min_length=1)` only bounded the top-level list length -- `items: [[]]` (or any all-empty nesting) passed the schema but flattened to zero leaf strings via `_nested_items`, silently planning zero ops instead of raising. Fixed with a `@model_validator(mode="after")` on `OutlineParams` that reuses `_nested_items` and raises when flattening yields no leaves. Added 2 build-layer tests plus 1 CLI and 1 MCP end-to-end test.

**Verification:** `cd server && uv run pytest -q` -- 1080 passed, coverage 96.44% (>=95% required; `cli/build.py` itself 97%). `uv run pyrefly check` -- 0 errors. `uv run ruff check` -- all checks passed.
