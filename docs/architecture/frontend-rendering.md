...
| `query` | `QueryBlock` |
| `todo` | `TodoCheckbox` |
| `pdf-embed`, `link` | `PdfEmbed` when `isPdfHref`; else `BlueskyEmbed` for a `link` whose href is a Bluesky post URL and equals its `text` (a bare URL; a markdown link with its own label stays an anchor); else an `<a target="_blank">` when `isSafeHref`; else plain text |
