...
it("renders backlink groups with breadcrumbs and loads more on demand", async () => {
  ...
  expect(fetchMock).toHaveBeenCalledWith(
    "/api/page/Machine%20Learning?bl_offset=1&bl_limit=20", READ_INIT);
...
it("refresh with an open filter panel refetches from offset 0 at limit 100 until complete", async () => {
  ...
  expect(fetchMock).toHaveBeenCalledWith(
    "/api/page/Claude?bl_offset=0&bl_limit=100", READ_INIT);
  expect(fetchMock).toHaveBeenCalledWith(
    "/api/page/Claude?bl_offset=1&bl_limit=100", READ_INIT);
...
it("a concurrent loadAll's groups survive a loadMore that started from a stale snapshot (pkm-3lqg)", async () => {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Show more" }));
  await vi.waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
    "/api/page/T?bl_offset=1&bl_limit=1", READ_INIT));
...
it("prevents opening the filter panel during refresh so stale load-all cannot start", async () => {
  ...
  await vi.waitFor(() => expect(fetchMock).toHaveBeenCalledWith(
    "/api/page/Claude?bl_offset=0&bl_limit=20", READ_INIT,
  ));
