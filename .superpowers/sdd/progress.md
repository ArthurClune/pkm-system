...
Merge phase: DONE. smft (7eb646e) + hx2w (0995b56) merged, full verify exit 0; wquz merged clean (no conflicts), full verify exit 0. Note: unrelated commit 02b2322 (pkm-srek PDF-viewer design spec, another session) landed on main mid-batch — excluded from batch judgment.

BATCH COMPLETE. Pushed at 4a0554a. Final whole-batch review (opus): ready to ship, zero Critical/Important; all three deferred Minors ACCEPTED (smft spy-assertions + positional boolean; wquz quote-prefix caret edge; hx2w reparent-focus untested at DOM level). One new Minor noted, no fix needed (Cmd-Enter collapses a textarea selection to caret — undocumented). Optional non-blocking follow-up idea: E2E assertion that focus survives a cross-parent Shift-Cmd move. Beans all completed; worktrees and branches removed. Prod NOT deployed (user not asked).
