...
Part of the contract rather than of either side's internals: a daily note
has no id of its own on the wire, so the CLI's `today`, the MCP server's
default page, and the server's own journal must all spell the same date
the same way or they address different pages."""
